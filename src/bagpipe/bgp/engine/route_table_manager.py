...
class Match(object):
    ...
    def __repr__(self):
        return "match:%s/%s,%s" % (self.afi or "*", self.safi or "*", self.routeTarget or "*")
    ...
    def __cmp__(self, other):
        ...
        return cmp((self_afi, self_safi, self_rt), (other_afi, other_safi, other_rt))
...
class RouteTableManager(Thread, LookingGlass):
    ...
    Workers subscribe to events by indicating AFI, SAFI, Route Targets in which they are interested.
    There can be workers responsible for handling services locally (e.g. a VPN instance) and workers that are BGP peers. 
    
    Though sources of routes are typically Workers, they don't need to be; any class can source a route.
    """
    class WorkersAndEntries(object):
        def __init__(self):
            self.workers = set()
            self.routeEntries = set() 
        def __repr__(self):
            return "workers: %s\nentries: %s" % (self.workers, self.routeEntries)
    
    def __init__(self):
        Thread.__init__(self, name="RouteTableManager")
        self.setDaemon(True)
        
        self._match2workersAndEntries = {}  # keys are Matches, values are WorkersAndEntries objects
        self._worker2matches = {}  # keys are Workers, values are Match objects
        self._source_nlri2entry = {}  # keys are (source,nlri) tuples, values are RouteEntry objects
        self._source2entries = {}  # dict: keys are event sources, each value is a set() of RouteEntry objects
    ...
    def _checkMatch2workersAndEntriesCleanup(self, match):
        ...
        if len(item.workers) == 0 and len(item.routeEntries) == 0:
    ...
    def _match2routeEntries(self, match, createIfNone=False, emptyListIfNone=True):
        if createIfNone:
            return self._match2workersAndEntriesLookupCreate(match).routeEntries
        try:
            return self._match2workersAndEntries[match].routeEntries
        except KeyError:
            if emptyListIfNone:
                return []
            else: raise
    ...
    def _source2entriesAddEntry(self, routeEntry):
        try:
            entries = self._source2entries[ routeEntry.source ]
        except KeyError:
            entries = set()
            self._source2entries[ routeEntry.source ] = entries
        entries.add(routeEntry)
        
    def _source2entriesRemoveEntry(self, routeEntry):
        try:
            entries = self._source2entries[ routeEntry.source ]
            log.debug("_source2entries[ entry.worker ] = %s " % entries)
            entries.discard(routeEntry)
        except KeyError:
            log.debug("(attempt at removing a non existing entry from self._source2entries[ entry.source ] : %s)" % routeEntry)
            pass

    def getWorkerSubscriptions(self, worker):
        """
        the returned entries should *not* be modified by caller (TODO: protect worker object by making their variables private)
        """
    ...
    def getWorkerRouteEntries(self, worker):
        """
        the returned entries should *not* be modified by caller (TODO: protect RouteEntry objects by making their variables private)
        """
    ...
    def _workerSubscribes(self, sub):
        ...
        for routeEntry in self._match2routeEntries(match):
            ...
            event = RouteEvent(RouteEvent.ADVERTISE, routeEntry)
            (shouldDispatch, reason) = self._shouldDispatch(event, worker)
            
            if shouldDispatch:
                # check if the routeEntry carries a routeTarget to which the worker 
                # was already subscribed
                for rt in routeEntry.routeTargets:
                    if Match(routeEntry.afi, routeEntry.safi, rt) in self._worker2matches[ worker ]:
                        (shouldDispatch, reason) = (False, "worker already had a subscription for this route")
                        break
            
            if shouldDispatch:
                log.info("Dispatching re-synthesized event for %s" % (routeEntry))
    ...
    def _workerUnsubscribes(self, sub):
        ...
        if sub.worker not in self._worker2matches:
            log.warning("worker %s unsubscribed from %s but was not tracked yet" % (sub.worker, match))
        else:
            try:
                self._worker2matches[ sub.worker ].remove(match)
            except KeyError:
                log.warning("worker %s unsubscribed from %s but this match was not tracked for this worker (should not happen, this is a bug)" % (sub.worker, match))
        
        # synthesize withdraw events
        for routeEntry in self._match2routeEntries(match, emptyListIfNone=True):

            intersect = set(self._matchesFor(routeEntry.afi,
                                         routeEntry.safi,
                                         routeEntry.routeTargets)).intersection(self._worker2matches[ sub.worker ])
            if len(intersect) > 0:
                log.debug("Will not synthesize withdraw event for %s, because worker subscribed to %s" % (routeEntry, intersect))
            else:
                log.debug("Found a routeEntry for this match: %s" % routeEntry)
                event = RouteEvent(RouteEvent.WITHDRAW, routeEntry)
                (shouldDispatch, reason) = self._shouldDispatch(event, sub.worker) 
                if shouldDispatch:
                    log.info("Dispatching re-synthesized event for %s" % (routeEntry))
        ...
        if match not in self._match2workersAndEntries:
            log.warning("worker %s unsubscribed from %s but we had no such subscription yet" % (sub.worker, match))
        else:
            try:
                self._match2workers(match).remove(sub.worker)
            except KeyError:
                log.warning("worker %s unsubscribed from %s but was not subscribed yet" % (sub.worker, match))
    ...
    def _propagateRouteEvent(self, routeEvent, exceptWorkers=None):
        '''Propagate routeEvent to workers subscribed to the route routetargets or wildcards, except the workers in 
        exceptWorkers. Returns the list of workers to which the event was propagated.'''
        ...
        for match in self._matchesFor(re.afi, re.safi, re.routeTargets):
            ...
            interestedWorkers = self._match2workers(match, emptyListIfNone=True)
            log.debug("   Workers interested in this match: %s" % interestedWorkers)

            for worker in interestedWorkers:
                (shouldDispatch, reason) = self._shouldDispatch(routeEvent, worker) 
                if shouldDispatch:
                    if not worker in exceptWorkers:
                        log.debug("Will dispatch event to %s: %s" % (worker, routeEvent))
                        targetWorkers.add(worker)
                    else:
                        log.debug("Decided not to dispatch to %s, based on exceptWorkers: %s" % (worker, routeEvent))
                else:
                    log.debug("Decided not to dispatch to %s: %s (%s)" % (worker, reason, routeEvent))
    ...
    def _receiveRouteEvent(self, routeEvent):
        ...
        routeEntry = routeEvent.routeEntry

        log.debug("Try to find a routeEntry from same peer with same nlri")
        try:
            replacedRouteEntry = self._source_nlri2entry[ (routeEntry.source, routeEntry.nlri) ]
        except KeyError:
            replacedRouteEntry = None
        
        log.debug("   Result: %s" % replacedRouteEntry)

        # replacedRouteEntry should be non-empty for a withdraw
        if replacedRouteEntry is None and (routeEvent.type == RouteEvent.WITHDRAW):
            raise Exception("WITHDRAW, but no route entry found that we could remove")
        ...
        if replacedRouteEntry is not None:
            ...
            removalEvent = RouteEvent(RouteEvent.WITHDRAW, replacedRouteEntry, routeEvent.source)
            
            self._propagateRouteEvent(removalEvent, workersAlreadyNotified)
            
            # Update match2routeEntries and source2entries for the replacedRoute           
            for match in self._matchesFor(replacedRouteEntry.afi, replacedRouteEntry.safi, replacedRouteEntry.routeTargets):
                try:
                    self._match2routeEntries(match).discard(replacedRouteEntry)
                except KeyError:
                    log.error("Trying to remove a route from a match, but match %s not found - not supposed to happen (route: %s)" % (match, replacedRouteEntry))
                self._checkMatch2workersAndEntriesCleanup(match)
            
            self._source2entriesRemoveEntry(replacedRouteEntry)
        
        if routeEvent.type == RouteEvent.ADVERTISE:
            # Update match2routeEntries and source2entries for the newly advertized route
            for match in self._matchesFor(routeEntry.afi, routeEntry.safi, routeEntry.routeTargets):
                self._match2routeEntries(match, createIfNone=True).add(routeEntry)
            self._source2entriesAddEntry(routeEntry)
            
            # Update _source_nlri2entry
            self._source_nlri2entry[(routeEntry.source, routeEntry.nlri)] = routeEntry
        else:  # WITHDRAW
            # Update _source_nlri2entry
            try:
                del self._source_nlri2entry[(routeEntry.source, routeEntry.nlri)]
            except KeyError:
                log.error("Withdraw, but nothing removed in _sourcenlri2entryRemove")
    ...
    def _shouldDispatch(self, routeEvent, targetWorker):
        ...
        the string contains the reason why the routeEvent should not be dispatched to targetWorker
        '''
    ...
    def _workerCleanup(self, worker):
        ...
        Consider this worker unsubscribed from all of its current subscriptions.
        '''
        ...
        if worker in self._source2entries:
            entries = self._source2entries[ worker ]
            log.info("  Preparing to withdraw %d routes that were advertised by worker" % len(entries))
            for routeEntry in entries:
                log.info("  Enqueue event to Withdraw route %s" % routeEntry)
                self.enqueue(RouteEvent(RouteEvent.WITHDRAW, routeEntry))
    ...
    def _dumpState(self):
        ...
        for source in self._source2entries.keys():
            ...
            for routeEntry in self._source2entries[ source ]:
                dump.append("    %s" % routeEntry)
        ...
        for match in matches:
                ...
                for re     in self._match2routeEntries(match):
    ...
    def getLGMap(self):
        return {
                "workers": (LGMap.COLLECTION, (self.getLGWorkerList, self.getLGWorkerFromPathItem)),
    ...
    def getLGRoutes(self, pathPrefix):
        ...
        match_IPVPN = Match(AFI(AFI.ipv4), SAFI(SAFI.mpls_vpn), Subscription.ANY_RT)
        match_EVPN = Match(AFI(AFI.l2vpn), SAFI(SAFI.evpn), Subscription.ANY_RT)
        ...
        for match in [match_IPVPN, match_EVPN, match_RTC]:
            ...
            if match in self._match2workersAndEntries:
                for routeEntry in self._match2routeEntries(match):
                    matchResult.append(routeEntry.getLookingGlassInfo(pathPrefix))
    ...
    def getAllRoutesButRTC(self):
        try:
            return filter(lambda re: not (re.afi == AFI(AFI.ipv4) and re.safi == SAFI(SAFI.rtc)),
                   self._match2workersAndEntries[ Match(Subscription.ANY_AFI, Subscription.ANY_SAFI, Subscription.ANY_RT) ].routeEntries)
        except KeyError:
            return []
    
    def getLocalRoutesCount(self):
        return reduce(lambda count, routeEntry: count + (not isinstance(routeEntry.source, BGPPeerWorker)),
                       self.getAllRoutesButRTC(),
                       0) 
    
    def getReceivedRoutesCount(self):
        return reduce(lambda count, routeEntry: count + isinstance(routeEntry.source, BGPPeerWorker),
                       self.getAllRoutesButRTC(),
                       0)
