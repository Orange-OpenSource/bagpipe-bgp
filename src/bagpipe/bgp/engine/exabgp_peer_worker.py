...
from exabgp.message.open import Open, RouterID, Capabilities
...
class MyBGPProtocol(Protocol):
    ...
    def new_open (self, restarted, asn4, config, enabledFamilies=[]):
        '''Same as exabgp.Protocol.new_open except that we advertise support for MPLS VPN and RTC'''
        ...
        o.capabilities[Capabilities.MULTIPROTOCOL_EXTENSIONS].remove((AFI(AFI.ipv4), SAFI(SAFI.flow_ipv4)))
        o.capabilities[Capabilities.MULTIPROTOCOL_EXTENSIONS].remove((AFI(AFI.ipv4), SAFI(SAFI.unicast)))
        o.capabilities[Capabilities.MULTIPROTOCOL_EXTENSIONS].remove((AFI(AFI.ipv6), SAFI(SAFI.unicast)))
...
class ExaBGPPeerWorker(BGPPeerWorker, LookingGlass):
    ...
    def __init__(self, bgpManager, name, peerAddress, config):
        ...
        self.peerCapabilites = None  # useful ??
    ...
    def _initiateConnection(self):
        self.log.debug("Initiate ExaBGP connection to %s from %s" % (self.peerAddress, self.localAddress))
        ...
        o = self.protocol.new_open(False, False, self.config, ExaBGPPeerWorker.enabledFamilies)
        ...
        while not self.shouldStop:
                ...
                if isinstance(message, NOP):
                    ...
                    if count > 20:
                        self.connection.close()  # FIXME: this should be moved to BGPPeerWorker in a more generic way (+ send Notify when needed)  
        ...
        # Hack to ease troubleshooting, have the real peer address appear in the logs when fakerr is used
        if received_open.router_id.ip != self.peerAddress:
            self.log.info("changing thread name from %s to BGP-x%s, based on the router-id advertized in Open (different from peerAddress == %s)" % (self.name, received_open.router_id.ip, self.peerAddress))
            self.name = "BGP-x%s" % received_open.router_id.ip
        
        try:
            mp_capabilities = received_open.capabilities[Capabilities.MULTIPROTOCOL_EXTENSIONS]
        except Exception:
            mp_capabilities = []
        
        # check that our peer advertized at least mpls_vpn and evpn capabilities
        self._activeFamilies = []
        for (afi, safi) in ExaBGPPeerWorker.enabledFamilies:
                if (afi, safi) not in mp_capabilities: 
                    self.log.warning("Peer does not advertise (%s,%s) capability" % (afi, safi))
                else:
                    self.log.info("Family (%s,%s) successfully negotiated with peer %s" % (afi, safi, self.peerAddress))
        ...
        if self.config['enable_rtc']:
            if (AFI(AFI.ipv4), SAFI(SAFI.rtc)) in mp_capabilities:
                self.log.info("RTC successfully enabled with peer %s" % self.peerAddress)
                self.rtc_active = True
            else:
                self.log.warning("enable_rtc True but peer not configured for RTC")
        ...
        if self.rtc_active:
            # subscribe to RTC routes, to be able to propagate them from internal workers to this peer
            self._subscribe(AFI(AFI.ipv4), SAFI(SAFI.rtc))
        else:
            # if we don't use RTC with our peer, then we need to see events for all routes of all active families, 
            # to be able to send them to him
            for (afi, safi) in self._activeFamilies:
                self._subscribe(afi, safi)
        # FIXME: all the above should be reverted (unsubscribe) when connection transitions to Idle state 
    ...
    def _receiveLoopFun(self):
        ...
        if not self._queue.empty():
            if self._stopLoops.isSet():
                self.log.info("stopLoops is set -> Close connection and Finish receive Loop")
            ...
            if message.routes:
                for route in message.routes:
                    ...
                    if AttributeID.EXTENDED_COMMUNITY in route.attributes:
                        rts = filter(lambda ecom:isinstance(ecom, RouteTarget),
                                     route.attributes[AttributeID.EXTENDED_COMMUNITY].communities
                                    )

                        if len(rts) == 0:
                            raise Exception("Unable to find any Route Targets s in the received route")
                    
                    routeEntry = self._newRouteEntry(route.nlri.afi, route.nlri.safi, rts, route.nlri, route.attributes)
                    ...
                    if (route.nlri.afi, route.nlri.safi) == (AFI(AFI.ipv4), SAFI(SAFI.rtc)):
                        ...
                        # if our peer subscribed to a Route Target, it means that we needs to send 
                        # him all routes of any AFI/SAFI carrying this Route Target.
                        for (afi, safi) in self._activeFamilies:
    ...
    def _send(self, data):
        ...
        self.log.debug("Sending %d bytes on socket to peer %s" % (len(data), self.peerAddress))
        try:
    ...
    def _updateForRouteEvent(self, event):
        route = Route(event.routeEntry.nlri)
        if event.type == event.ADVERTISE:
            self.log.info("Generate UPDATE message: %s" % route)
            route.attributes = event.routeEntry.attributes
            try:
                return Update([route]).update(False, self.config['my_as'], self.config['my_as'])  # AS numbers are probably useless to set here
            except Exception as e:
                self.log.error("Exception while generating message for route %s: %s" % (route, e))
                # if self.log.isEnabledFor(self.log.debug):
                self.log.warning("%s" % traceback.format_exc())
                return ''
        
        elif event.type == event.WITHDRAW:
            self.log.info("Generate WITHDRAW message: %s" % route)
            return Update([route]).withdraw(False, self.config['my_as'], self.config['my_as'])  # AS numbers are probably useless to set here
    ...
    def getLookingGlassLocalInfo(self, pathPrefix):
        return {
                ...
                # "activeFamilies": self._activeFamilies,
                }
