...
class BGPPeerWorker(Worker, Thread, LookingGlass):
    ...
    def __init__(self, bgpManager, name, peerAddress):
        ...
        # its up to subclasses to call setHoldTime again to set holdtime based on value advertized by peer
        self._setHoldTime(DEFAULT_HOLDTIME) 
        
        self._stopLoops = Event() # used to stop receiveThread
        self.shouldStop = False   # used to track that we've been told to terminate
    ...
    def _onEvent(self, event):
        ...
        if   event == Init:
            ...
            if (self.fsm.state == FSM.Established):
                ...
                # FIXME: this is possibly not correct yet: why did we received this event  ? what do we do with it ? 
                raise Exception("cannot process routeEvent in '%s' state" % self.fsm.state)
    ...
    def _initiateConnectionAndThreads(self):
        ...
        try:
            ...
            self.log.warning("%s while initiating connection: %s" % (e.__class__.__name__,e))
            self.enqueue(ReInit)
            return
            # FIXME: we should transition to Active or Idle state depending how many times we already tried
        ...
        self.receiveThread = Thread(target=self._receiveLoop, name="%s:receiveLoop" % self.name)
        self.receiveThread.start()

    def _initiateConnection(self):
        '''
        Abstract method.
        The implementation will initiated the connection to the BGP peer, do the initial BGP handshake (send Open, 
        receive Open, send first KeepAlive, receive first KeepAlive) and track the intermediate FSM states (OpenSent, OpenConfirm).
        '''
    ...
    def _reinitiate(self):
        ...
        # TODO(tmmorin): replace with a timer that injects an event, so that we avoid sleeping which make us miss any stop event
        sleep(10)
    ...
    def _receiveLoop(self):
        ...
        while not self._stopLoops.isSet():
            try:
                ...
                if loopResult == 0:
                    self.log.info("receiveLoopFun returned 0, aborting receiveLoop")
                    break
                elif loopResult == 2:
                    self.log.warning("receiveLoopFun returned 2 (error), aborting receiveLoop and reinitializing")
                    self.enqueue(ReInit)  # FIXME: use (Worker.)enqueueHighPriority so that it's treated before other events
                    break
                else:
                    # everything went fine
                    pass
            except Exception as e:
                self.log.error("Error: %s (=> aborting receiveLoop and reinitializing)" % e)
    ...
    def initSendKeepAliveTimer(self):
        ...
        self.sendKATimer = Timer(self.katPeriod, BGPPeerWorker.sendKeepAliveTrigger, [self])
    ...
    def initKeepAliveReceptionTimer(self):
        self.log.debug("Init Keepalive reception timer (%ds)" % (self.katExpiryTime))
        self.KAReceptionTimer = Timer(self.katExpiryTime, BGPPeerWorker.onKeepAliveExpired, [self])
